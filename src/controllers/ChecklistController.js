...
class ChecklistController {
  ...
  async deleteChecklistById(req, res) {
    Checklist.findOne({
      ...
      .then(async function (rows) {
        if (rows && rows.dataValues) {
          db.query(`DELETE FROM checklists WHERE id = ${req.params.id}`, { type: db.DELETE });
          db.query(`DELETE FROM tasks WHERE checklist_id = ${req.params.id}`, { type: db.DELETE });
          return res.redirect("/checklists");
        } else {
          return res.send("Checklist not found!");
        }
